...
def detect(opt):
  source, yolo_weights, deep_sort_weights,imgsz,conf_thres,iou_thres= \
    opt.source, opt.yolo_weights, opt.deep_sort_weights,opt.img_size, opt.conf_thres, opt.iou_thres
  ...
  while True:
    ...
    try:
      ort_inputs = {session.get_inputs()[0].name: input_}
      ort_outs = session.run(None, ort_inputs)
      pre = torch.FloatTensor(ort_outs[0])
      pred = non_max_suppression(pre)
      dets =pred[0]
    ...
    for det in dets:
      ...
      x1, y1, x2, y2, score,cls = (det / scale).astype(np.float32)
